...
export async function setUserPreference(teamId, userId, prefs) {
  await redis.set(`prefs:${teamId}:${userId}`, JSON.stringify(prefs));
}

export async function getUserPreference(teamId, userId) {
  const raw = await redis.get(`prefs:${teamId}:${userId}`);
  if (!raw) return null;
  return typeof raw === 'string' ? JSON.parse(raw) : raw;
}
